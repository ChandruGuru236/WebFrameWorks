...
import com.example.day6cw3.service.ChandruGuruProductService;
...
public class ChandruGuruProductController {
    ...
    public ResponseEntity<List<ChandruGuruProduct>> getMethodName(@PathVariable("offset") int offset,@PathVariable("pagesize") int size) {
        List<ChandruGuruProduct> list = productService.pagination(offset,size);
        if(list.size() == 0)
        {
    ...
    public ResponseEntity<List<ChandruGuruProduct>> getSortedPagination(@PathVariable("offset") int offset,@PathVariable("pagesize") int size,@PathVariable("field") String field) {
        List<ChandruGuruProduct> list = productService.sortPagination(offset,size,field);
        if(list.size() == 0)
        {
            return new ResponseEntity<>(null,HttpStatus.NOT_FOUND);
        }
    ...
    public ResponseEntity<ChandruGuruProduct> getMethodName(@PathVariable("productId") int id) {
        ChandruGuruProduct product = productService.getProductById(id);
        if(product==null)
        {
            return new ResponseEntity<>(null,HttpStatus.NOT_FOUND);
        }
